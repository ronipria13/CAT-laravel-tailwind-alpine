...
class UpdateSegmentRequest extends FormRequest
{
    ...
    {   
        ...
        $segment = Segment::find(request()->segment(3));
        if($segment->segment_code == request('segment_code')){
            $rules['segment_code'] = 'required|max:10';
        }else{
            $rules['segment_code'] = 'required|unique:segments,segment_code|max:10';
        }
